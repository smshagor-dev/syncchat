...
module.exports = (socket) => {
  ...
  socket.on(
    ...
    async ({
      ...
      let config = null;
      let normalizedMediaType = mediaType === 'video' ? 'video' : 'audio';
      ...
      try {
        ...
        if (reservedState) {
          await releaseCall(reservedState).catch(() => {});
        }
        if (!historyCreated) {
          await safeFailedHistory({
            callId,
  ...
  socket.on('call/join', async ({ callId, roomId, userId, mediaType }) => {
    ...
    try {
      ...
      if (!state) {
        emitCallError(userId, roomId, 'Call not found', 'CALL_NOT_FOUND', callId);
        return;
      }
  ...
  socket.on(
    'call/reject',
    async ({ callId, roomId, fromUserId, toUserId }) => {
