...
const waitForCallState = async ({ roomId, fromUserId }) => {
  for (let attempt = 0; attempt < 8; attempt += 1) {
    const state = await getActiveCallByRoom(roomId).catch(() => null);
    if (state?.initiatorId === fromUserId) return state;
    await sleep(125);
  }
  return null;
};
...
module.exports = (socket) => {
  socket.on('call/start', async ({ roomId, fromUserId }) => {
    ...
    try {
      const state = await waitForCallState({ roomId, fromUserId });
      if (!state?.callId || !Array.isArray(state.recipientsId) || !state.recipientsId.length) {
        return;
      }

      const call = buildIncomingCall(state);
      const title = call.mediaType === 'video' ? 'Video call' : 'Voice call';
      const caller = call.fromName || (call.fromUsername ? `@${call.fromUsername}` : 'Someone');
