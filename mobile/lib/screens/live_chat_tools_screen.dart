...
class _LiveChatToolsScreenState extends State<LiveChatToolsScreen> {
  ...
  Future<void> loadTopics() async {
    ...
    try {
      ...
      if (!mounted) return;
      setState(() {
        topics = mapList(response.payload);
        loading = false;
      });
    } on Object catch (failure) {
      fail(failure);
  ...
  Future<void> createTopic() async {
    ...
    try {
      await context.services.api.post('/chat-v2/topics/$roomId', body: {'name': name});
      topicName.clear();
      await loadTopics();
    } on Object catch (failure) {
      fail(failure);
  ...
  Widget searchBody() => ListView(
    ...
    children: [
      ...
      TextField(
        ...
        controller: TextEditingController(text: targetLanguage),
  ...
  Widget topicsBody() {
    ...
    return ListView(
      padding: const EdgeInsets.all(14),
      children: [
        Row(children: [Expanded(child: TextField(controller: topicName, decoration: const InputDecoration(labelText: 'New topic name'))), const SizedBox(width: 8), FilledButton(onPressed: createTopic, child: const Text('Create'))]),
        const SizedBox(height: 14),
        ...topics.map((item) => Card(color: context.panel, elevation: 0, child: ListTile(leading: const Icon(Icons.tag_rounded), title: Text(item['name']?.toString() ?? 'Topic'), subtitle: Text(item['closed'] == true ? 'Closed' : 'Open'), trailing: item['pinned'] == true ? const Icon(Icons.push_pin_rounded) : null))),
      ],
    );
  }
  ...
  Widget outboxBody() => ListView(
    ...
    children: [
      Card(
        ...
        elevation: 0,
        child: ListTile(
          leading: const Icon(Icons.cloud_sync_outlined, color: SyncColors.sky),
          title: const Text('Mobile delivery recovery', style: TextStyle(fontWeight: FontWeight.w900)),
          subtitle: const Text('Live rooms use stable client message IDs plus reconnect/sync recovery. Failed optimistic sends remain visible in the conversation for retry awareness.'),
        ),
      ),
      if (roomId.isNotEmpty) ...[
        ...
        Card(
          ...
          child: ListTile(
            ...
            subtitle: const Text('Open the schedule menu in the message composer to manage send-later, when-online, and recurring messages.'),
          ),
        ),
      ],
    ],
