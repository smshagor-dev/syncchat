...
class LiveResumableUploadScreen extends StatefulWidget {
  ...
  @override
  State<LiveResumableUploadScreen> createState() => _LiveResumableUploadScreenState();
}

class _LiveResumableUploadScreenState extends State<LiveResumableUploadScreen> {
  ...
  Future<void> _bootstrap() async {
    try {
      final result = await context.services.inbox.list();
      final available = result
          .where((item) => item['roomId']?.toString().trim().isNotEmpty == true)
      ...
      String? restoredPath = draft?['filePath']?.toString();
      if (restoredPath != null && !await File(restoredPath).exists()) {
        restoredPath = null;
        draft = null;
        await _storage.delete(key: _draftKey).catchError((_) {});
      }

      if (!mounted) return;
      setState(() {
        rooms = available;
        selectedRoomId = available.any((item) => item['roomId']?.toString() == draft?['roomId']?.toString())
            ? draft?['roomId']?.toString()
            : (available.isEmpty ? null : available.first['roomId']?.toString());
        filePath = restoredPath;
        filename = draft?['filename']?.toString();
        fileSize = (draft?['fileSize'] as num?)?.toInt() ?? 0;
        uploadId = draft?['uploadId']?.toString();
        offset = (draft?['offset'] as num?)?.toInt() ?? 0;
        chunkSize = (draft?['chunkSize'] as num?)?.toInt() ?? chunkSize;
        paused = uploadId != null && filePath != null;
        loading = false;
      });
      if (uploadId != null && filePath != null) await _syncServerStatus(silent: true);
  ...
  Future<void> _pickFile() async {
    ...
    if (path == null || path.isEmpty) {
      setState(() => error = 'The selected file is not accessible on this device.');
      return;
    }
  ...
  Future<void> _createSession() async {
    ...
    if (id.isEmpty) {
      throw const ApiException(statusCode: 500, message: 'Upload session ID is missing.');
    }
    if (!mounted) return;
    setState(() {
      uploadId = id;
      offset = (payload['offset'] as num?)?.toInt() ?? 0;
      chunkSize = (payload['chunkSize'] as num?)?.toInt() ?? 4 * 1024 * 1024;
    });
    await _persistDraft();
  }
  ...
  Future<void> _syncServerStatus({bool silent = false}) async {
    ...
    final response = await context.services.api.get('/chats/uploads/resumable/$id');
    final payload = _map(response.payload);
    if (!mounted) return;
    setState(() {
  ...
  Future<void> _pump() async {
    ...
    if (actualSize != fileSize) {
      throw const ApiException(
        statusCode: 409,
        message: 'The local file changed after this upload started. Choose it again.',
      );
    }
    ...
    try {
      ...
      while (offset < fileSize && !paused && !cancelRequested) {
        ...
        if (chunk.isEmpty) {
          throw const ApiException(statusCode: 500, message: 'Unexpected end of local file.');
        }
        final before = offset;
        final payload = await _sendChunk(id, before, chunk);
        final nextOffset = (payload['offset'] as num?)?.toInt() ?? (before + chunk.length);
        if (nextOffset <= before) {
          throw const ApiException(statusCode: 409, message: 'Server did not advance the upload offset.');
        }
        if (!mounted) return;
        final elapsed = DateTime.now().difference(startedAt).inMilliseconds / 1000;
        final transferred = nextOffset - startOffset;
        final speed = elapsed > 0 ? transferred / elapsed : 0.0;
        setState(() {
          offset = nextOffset;
          bytesPerSecond = speed;
          eta = speed > 0 ? Duration(seconds: ((fileSize - nextOffset) / speed).ceil()) : null;
        });
        await _persistDraft();
      }
    } finally {
    ...
    if (room.isEmpty) {
      throw const ApiException(statusCode: 404, message: 'Destination chat is no longer available.');
    }
    await context.services.chat.sendAttachment(inbox: room, file: uploadedFile);
    await _storage.delete(key: _draftKey).catchError((_) {});
    if (!mounted) return;
    setState(() {
      uploading = false;
  ...
  Future<Map<String, dynamic>> _sendChunk(
    ...
    for (var attempt = 0; attempt < 3; attempt++) {
      ...
      try {
        ...
        final request = http.Request(
          'PUT',
          context.services.config.apiUri('/chats/uploads/resumable/$id/chunk'),
        )
          ..headers.addAll({
        ...
        if (streamed.statusCode < 200 || streamed.statusCode >= 300 || map['success'] != true) {
          throw ApiException(
            statusCode: streamed.statusCode,
            message: map['message']?.toString() ?? 'Upload chunk failed.',
            payload: map['payload'],
          );
        }
        return _map(map['payload']);
      } on Object catch (failure) {
        activeClient?.close();
        activeClient = null;
    ...
    throw ApiException(statusCode: 0, message: _message(lastFailure ?? Exception('Chunk upload failed.')));
  }
  ...
  Future<void> _cancel() async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        content: const Text('Uploaded temporary chunks will be deleted from the server.'),
        actions: [
          TextButton(onPressed: () => Navigator.pop(dialogContext, false), child: const Text('Keep upload')),
          FilledButton(onPressed: () => Navigator.pop(dialogContext, true), child: const Text('Cancel upload')),
        ],
      ),
    );
  ...
  Widget _intro() => Container(
        ...
        child: Row(
          ...
          children: [
            const Icon(Icons.cloud_upload_outlined, color: SyncColors.sky),
  ...
  Widget _destination() => Card(
        ...
        child: Padding(
          ...
          child: DropdownButtonFormField<String>(
            initialValue: rooms.any((item) => item['roomId']?.toString() == selectedRoomId) ? selectedRoomId : null,
            isExpanded: true,
            ...
            items: rooms.map((room) => DropdownMenuItem(
              ...
              child: Text(_roomName(room), maxLines: 1, overflow: TextOverflow.ellipsis),
            )).toList(growable: false),
  ...
  Widget _fileCard() => Card(
        ...
        child: ListTile(
          ...
          leading: const Icon(Icons.insert_drive_file_outlined, color: SyncColors.sky, size: 30),
          title: Text(filename ?? 'Choose a file', maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontWeight: FontWeight.w900)),
          subtitle: Text(filePath == null ? 'Resumable upload supports the server-configured chat limit.' : _bytes(fileSize)),
          trailing: IconButton(
  ...
  Widget _progressCard() {
    final progress = fileSize > 0 ? (offset / fileSize).clamp(0.0, 1.0) : 0.0;
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Expanded(child: Text(state, style: const TextStyle(fontWeight: FontWeight.w900))),
              Text('${(progress * 100).toStringAsFixed(1)}%', style: const TextStyle(fontWeight: FontWeight.w900, color: SyncColors.sky)),
            ],
          ),
          const SizedBox(height: 10),
          LinearProgressIndicator(value: progress, minHeight: 8, borderRadius: BorderRadius.circular(8)),
          const SizedBox(height: 10),
          Text('${_bytes(offset)} of ${_bytes(fileSize)}', style: TextStyle(color: context.muted, fontSize: 12)),
          const SizedBox(height: 8),
          Wrap(
            spacing: 8,
            runSpacing: 8,
            children: [
              _stat(Icons.speed_rounded, bytesPerSecond > 0 ? '${_bytes(bytesPerSecond.round())}/s' : '—'),
              _stat(Icons.timer_outlined, eta == null ? 'ETA —' : 'ETA ${_duration(eta!)}'),
              _stat(Icons.layers_outlined, '${_bytes(chunkSize)} chunks'),
            ],
          ),
        ],
      ),
            ...
            child: FilledButton.icon(
              onPressed: completed || cancelling
                  ...
                  : uploading
                      ? _pause
                      : _startOrResume,
              icon: Icon(uploading ? Icons.pause_rounded : (uploadId != null ? Icons.play_arrow_rounded : Icons.cloud_upload_outlined)),
              label: Text(uploading ? 'Pause' : (uploadId != null ? 'Resume' : 'Upload & send')),
            ),
          ),
          const SizedBox(width: 10),
          OutlinedButton.icon(
            onPressed: uploadId == null || cancelling || completed ? null : _cancel,
            icon: cancelling
  ...
  String _mimeHint(String name) {
    ...
    if (RegExp(r'\.(jpg|jpeg|png|gif|webp|avif|heic)$').hasMatch(lower)) return 'image/*';
    if (RegExp(r'\.(mp4|mov|mkv|webm|m4v)$').hasMatch(lower)) return 'video/*';
    if (RegExp(r'\.(mp3|m4a|aac|wav|ogg|flac)$').hasMatch(lower)) return 'audio/*';
